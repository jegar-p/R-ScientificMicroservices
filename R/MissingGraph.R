#' Get a graph of missing values in a dataframe by row and column
#'
#' @description MissingGraph takes a dataframe and returns a png representing the row and column of missing data.
...
#' @examples 
#' \dontrun{MissingGraph(missing_data = data.frame(c(10, 11, 12), c(50, NA, 50), c(NA, NA, 14), c(50, 50, NA)), email = 'info@scientificmicroservices.com')}
#' @export
MissingGraph <- function(missing_data, email = "", key = "")
  # Get a summary of the missing data in your dataset by row and column.
{
  if(key == "")
  {key <- test_key()}
  x <- jsonlite::fromJSON(rawToChar(httr::POST("https://api.scientificmicroservices.com/missinggraph",
